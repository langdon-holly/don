...
function indexToLineColumn(index, string) {
  ...
  throw new RangeError("indexToLineColumn: index=" + index + " is out of bounds");}
...
fs.readFile(program.args[0], 'utf8', function(err, data) {
  ...
  if (parsed[0]) {
    ...
    var errAt = parsed[1];
    if (errAt == -1) console.log("Syntax error: " + program.args[0] + " should be at least " + (Array.from(data).length + 1) + " characters");
